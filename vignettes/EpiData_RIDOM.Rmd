...
Eine `Problems.csv` Datei mit solchen "gelösten Problemen", also angepassten SurvNet Aktenzeichen lässt sich wiederum als neue Import-Datei (`LIMS_link_file`) verwenden.

```{r problems2}
solvedProblems <- create_Epidata(
  LIMS_link_file = "O:/Abteilung Humanmedizin (AHM)/Referat 32/32_6/14_EpiDaten/LIMS Export Merger/Problems.csv")
```
`solvedProblems` ist hier leer, da keine manuelle Korrektur in `Problems.csv` vorgenommen wurde. 
